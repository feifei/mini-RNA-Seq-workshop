...
echo $GENOME
STAR --runMode genomeGenerate --genomeDir ${MAPPING_DIR}/genome --genomeFastaFiles $GENOME \
	--sjdbGTFfile ${GTF_FILE} --runThreadN 16 --sjdbOverhang 125
...
for READ1 in ${READ1_FILES} 
	do
		...
		echo ${prefix}
		echo $READ2
		STAR --genomeDir ${MAPPING_DIR}/genome --readFilesIn $READ1 $READ2 --readFilesCommand gunzip -c \
			--runThreadN 16 --outFileNamePrefix ${MAPPING_DIR}/${prefix}_ --quantMode GeneCounts
