...
#
# STAR --runMode genomeGenerate --genomeDir ${WORK_DIR}genomes --genomeFastaFiles ${WORK_DIR}merged.fasta --runThreadN 8 --sjdbGTFfile ${WORK_DIR}merged.gtf --sjdbOverhang 125 --genomeChrBinNbits 15

for j in `seq 1 33`
do
    prefix=star_${j}
    READS=${WORK_DIR}${j}_S*_L00*_R*_001.fastq.gz
    STAR --genomeDir ${WORK_DIR}/genomes --readFilesIn $READS --readFilesCommand gunzip -c --runThreadN 16 --outFileNamePrefix ${WORK_DIR}${prefix}_ --quantMode GeneCounts
